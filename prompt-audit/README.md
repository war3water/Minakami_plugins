...
- **Reference refresh.** The `references/` no-op and anti-pattern lists shift as model capability evolves. Refreshing them is a normal version bump, gated by the same eval sweep — a reference change should visibly move at least one expected finding's basis tag or severity, and regress nothing.
